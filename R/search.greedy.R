...
greedy.search <- function(attributes, eval.fun, forward = TRUE) {
	...
	repeat {
		...
		lst.attrib <- c(lst.attrib,  local_best)
		lst.perf   <- c(lst.perf, children_results)
	...
	return(list(features=attributes[as.logical(best$attrs)],lst.attrib=lst.attrib,lst.perf=lst.perf))
